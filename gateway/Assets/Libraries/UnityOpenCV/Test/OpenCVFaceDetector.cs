...
public class OpenCVFaceDetector : IFaceDetector
{
	...
	void _threadProcess()
	{
		if (!_inited) {
			...
			_detector = new UnityOpenCVFaceDetectorAPI (_config.cascadePath,_config.resize,_config.scaler,_config.minNeighbors,_config.minSize,_config.maxSize);
		}
		while (!_isDone) {
