...
public class LocalCameraVideoPlayer : BaseVideoPlayer {
    ...
    public GstImageInfo.EPixelFormat Format = GstImageInfo.EPixelFormat.EPixel_I420;
...
//		pipeline = "filesrc location=~/Documents/Projects/BeyondAR/Equirectangular_projection_SW.jpg ! jpegdec ! videoconvert ! imagefreeze ! videoconvert ! imagefreeze ! videoconvert ! video/x-raw,format=I420 ! appsink name=videoSink sync=true";
		...
		pipeline = "ksvideosrc device-index="+ CameraIndex .ToString()+ " ! video/x-raw,width="+Width.ToString()+",height=" + Height.ToString() + " ! videoconvert ! video/x-raw,format=I420 ! videoflip method=5 ! appsink name=videoSink sync=true";
