...
public class GStreamerCore {
	...
	[DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
	extern static private IntPtr  mray_GetTextureData( IntPtr str );

	[DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
	extern static private void mray_ReleaseTextureData( IntPtr str );
	...
	public static IntPtr GetTextureData(Texture2D tex)
	{
		if (tex == null)
			return IntPtr.Zero;
		return mray_GetTextureData (tex.GetNativeTexturePtr());
	}

	public static void ReleaseTextureData(Texture2D tex)
	{
		if (tex == null)
			return ;
		mray_ReleaseTextureData(tex.GetNativeTexturePtr());
	}
